...
로그아웃 요청이 시작되면 새로운 토큰 갱신을 시작하지 않는다. 이미 진행 중인 API-004가 있으면 그 결과와 `Set-Cookie` 반영이 끝날 때까지 기다린 뒤 API-006을 호출한다. 로그아웃 의사가 설정된 뒤에는 API-004를 기다리던 원 요청도 재시도하지 않는다.
...
프론트엔드는 로그아웃과 토큰 갱신의 응답이 경쟁하지 않도록 두 요청을 같은 인증 요청 흐름에서 직렬화한다. 진행 중인 API-004가 있으면 완료와 Cookie 반영을 기다린 뒤 API-006을 호출하고, 로그아웃을 시작한 뒤에는 새 API-004나 인증 요청 재시도를 시작하지 않는다.
