...
- 첨삭 상태 SSE 연결 시 모든 자기소개서의 현재 표시 상태 스냅샷을 먼저 수신하고 이후 시작, 완료, 실패처럼 메인 화면에 필요한 변경을 `changeType`과 함께 실시간으로 수신한다.
- 스냅샷과 변경 이벤트는 `displayStatus`와 최신 첨삭 버전 ID를 하나의 목록 상태로 함께 갱신하며, 연결 등록과 스냅샷 전송 사이의 변경 이벤트를 유실하지 않는다.
- 프론트엔드는 내부 `CoverLetter.status`와 Job 상태를 조합하지 않는다. 재첨삭 실패 시 기존 결과 접근이 가능한 `displayStatus=REVIEWED`를 유지하면서 `changeType=REVIEW_FAILED`로 실패를 식별한다.
...
- 최종 실패한 Job의 임시 문항 결과는 사용자-facing API에서 숨긴다.

### Acceptance Criteria

- 제출 성공 시 자기소개서 상태와 LLM Job 상태가 함께 갱신된다.
- Job 상태 조회와 SSE 스트림으로 진행 상태를 확인할 수 있다.
- SSE 연결 시 이미 완성된 문항 결과 스냅샷을 먼저 전달해 상세 조회와 스트림 연결 사이의 누락을 방지한다.
