...
| `api/rewrite-api-documentation.xlsx` | 전체 API와 필드 의미, 구현 여부, API별 최근 변경 일시. Google Drive 공유본의 기준 원본 | API 전체 현황 확인, API 추가/변경 |
...
- `api/rewrite-api-documentation.xlsx`를 변경할 때는 `codex-workflow.md`의 Google Drive 동기화 절차를 따른다.
...
| API 추가, 삭제, path/request/response/error/validation/상태/구현 여부 변경 | `api/README.md`, 관련 `api/*.md`, `api/rewrite-api-documentation.xlsx`, Google Drive 미러, 필요 시 `requirements.md`, `decisions/`, `status.md` | Markdown·Excel·Drive의 API ID, Related Requirement, 상세 명세와 상태가 모두 일치하는가 |
