...
- API 계약 확인: Swagger UI와 `docs/api/README.md`
...
| `api/*.md` | 도메인별 API 보완 계약과 교차 API 흐름 | 특정 도메인 API 구현/수정 |
...
- API 계약 또는 구현 상태를 바꾸면 controller·DTO·`@RewriteApi`, OpenAPI 통합 테스트, 관련 Markdown과 Notion 미러를 `API ID`로 함께 갱신한다.
- API 관련 작업은 관련 `API ID`의 생성 OpenAPI와 저장소 문서를 먼저 확인한다. 활성 API는 모두 `@RewriteApi`를 사용하며 Notion은 생성 OpenAPI와 저장소 문서의 보조 미러로 확인한다.
...
- 코드나 문서를 변경할 때는 `codex-workflow.md`의 위험도 기준과 변경 영향 완결성 절차를 따른다.
...
| 기능 검증 완료 | `status.md`, `api/README.md` | 관련 검증 통과와 리뷰 또는 사용자 승인 근거가 `Verification Evidence`에 남았는가 |
| API 추가, 삭제, path/request/response/error/validation/상태/구현 여부 변경 | controller·DTO·`@RewriteApi`, OpenAPI 테스트, `api/README.md`, 관련 `api/*.md`, Notion 미러, 필요 시 `requirements.md`, `decisions/`, `status.md` | 생성 OpenAPI·코드·저장소 문서·Notion의 API ID와 계약이 모두 일치하는가 |
...
| 기능 상태를 추적하는 GitHub issue 생성 | 관련 항목이 있는 경우 `status.md` Related Issue/PR | 관련 REQ/API/Decision이 issue 본문과 기존 status 항목에 연결됐는가 |
| 문서 구조 또는 작업 절차 변경 | `README.md`, `docs/README.md`, `codex-workflow.md`, 필요 시 `AGENTS.md` | 구식 문서 경로나 중복 기준 문서가 남지 않았는가 |

## Completion Checklist

이 체크리스트를 작업 완료와 PR 초안 작성 전 변경 완결성의 단일 기준으로 사용한다.

- 해당하는 `REQ-*`, API ID, Decision ID를 식별했다. 해당하지 않으면 그 이유가 명확하다.
- 코드 변경과 문서 상태가 충돌하지 않는다.
- 변경 유형에 맞는 문서를 Change Impact Matrix 기준으로 갱신했다.
- `Implemented` 또는 `Verified`로 상태를 변경했다면 `Verification Evidence`를 함께 갱신했다.
- 실행한 검증과 결과 또는 실행하지 못한 이유를 기록했다.
- 예상 영향 범위와 실제 diff를 대조했고, 미반영 항목에는 근거가 있다.
