...
| REQ-008 | 실제 인증 경계 | In Progress | Medium | API-001 - API-006 | [#94](https://github.com/Rewrite-Team/Rewrite-BE/issues/94), [#96](https://github.com/Rewrite-Team/Rewrite-BE/issues/96) | 인증·CSRF 독립 리뷰, `AuthIntegrationTest`, `TokenRefreshControllerTest`, `CsrfTokenServiceTest`, `RestKakaoClientTest`, `KakaoLoginServiceTest`, `./gradlew clean check` | API-001·002·003·004·005를 구현했다. API-004는 refresh token 원자적 rotation과 실패별 Cookie 정책까지 반영했으며, API-006 로그아웃은 후속 이슈로 남는다. |
...
로그인 제외 제품 기능은 구현됐다. REQ-008의 API-001·002·003·005는 구현·검증됐고 API-004 토큰 갱신은 구현됐다. 다음 인증 slice는 API-006 로그아웃이다.
