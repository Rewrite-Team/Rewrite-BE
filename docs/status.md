...
| REQ-003 | 자기소개서 기본 CRUD | In Progress | High | API-007, API-008, API-012, API-013, API-030 | [#5](https://github.com/Rewrite-Team/Rewrite-BE/issues/5), [#18](https://github.com/Rewrite-Team/Rewrite-BE/issues/18), [#28](https://github.com/Rewrite-Team/Rewrite-BE/issues/28), [#30](https://github.com/Rewrite-Team/Rewrite-BE/issues/30), [#84](https://github.com/Rewrite-Team/Rewrite-BE/issues/84), [#90](https://github.com/Rewrite-Team/Rewrite-BE/issues/90), [#92](https://github.com/Rewrite-Team/Rewrite-BE/issues/92), [PR #15](https://github.com/Rewrite-Team/Rewrite-BE/pull/15) | `CoverLetterServiceTest`, `CoverLetterControllerTest`, `CoverLetterRepositoryTest`, API-007 독립 설계·테스트 리뷰, `./gradlew test`, `./gradlew check` | API-007은 상태 필터 없이 전체 목록을 조회하고 저장된 `CoverLetter.status`를 `displayStatus`로 반환한다. API-008은 `id`, API-013은 `success` 단일 응답으로 구현했다. API-012 상태 공통 상세와 API-030 사용자 단일 SSE는 구현 필요 |
| REQ-004 | 자기소개서 등록 step 저장 | In Progress | High | API-009, API-010, API-011, API-012, API-014 | [#32](https://github.com/Rewrite-Team/Rewrite-BE/issues/32), [#34](https://github.com/Rewrite-Team/Rewrite-BE/issues/34), [#36](https://github.com/Rewrite-Team/Rewrite-BE/issues/36), [#90](https://github.com/Rewrite-Team/Rewrite-BE/issues/90) | `CoverLetterServiceTest`, `CoverLetterControllerTest`, `CoverLetterQuestionRepositoryTest`, `GlobalExceptionHandlerTest`, `./gradlew test`, `./gradlew check` | API-009~011 nullable WRITING 임시저장과 API-014 최종 필수값 검증은 구현됨. 개발 H2 DB를 새 schema로 재생성했으며 API-012 미완성 데이터 복구는 구현 필요 |
...
| 메인 첨삭 상태 실시간 갱신 | REQ-003, REQ-005 | API-030 | 저장된 `CoverLetter.status`를 사용하는 전체 스냅샷·단건 변경 SSE 구현 |
