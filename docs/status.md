...
| REQ-008 | 실제 인증 경계 | In Progress | Medium | API-001 - API-006 | [#94](https://github.com/Rewrite-Team/Rewrite-BE/issues/94) | 인증·CSRF 독립 리뷰, `AuthIntegrationTest`, `CsrfTokenServiceTest`, `RestKakaoClientTest`, `KakaoLoginServiceTest`, `./gradlew clean check` | API-001·002·003·005를 구현·검증했다. API-004 토큰 갱신과 API-006 로그아웃은 후속 이슈로 남는다. |
...
로그인 제외 제품 기능은 구현됐다. REQ-008의 API-001·002·003·005는 구현·검증됐으며, 다음 인증 slice는 API-004 토큰 갱신과 API-006 로그아웃이다.
