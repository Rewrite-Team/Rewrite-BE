...
| REQ-010 | AI 면접 | In Progress | Medium | API-022, API-023, API-025 - API-029 | [#62](https://github.com/Rewrite-Team/Rewrite-BE/issues/62), [#64](https://github.com/Rewrite-Team/Rewrite-BE/issues/64) | `InterviewSessionRepositoryTest`, `InterviewQuestionRepositoryTest`, `InterviewServiceTest`, `InterviewControllerTest`, `OpenAiInterviewQuestionGenerationClientTest`, `InterviewQuestionGenerationJobWorkerTest`, `InterviewQuestionGenerationJobEventIntegrationTest`, `LlmJobRepositoryTest`, `./gradlew test`, `./gradlew check` | API-022 면접 세션 시작, PENDING 초기 질문 생성 Job 생성, OpenAI 초기 질문 생성 client와 after-commit worker, 질문 5개 저장 구현됨 |
...
|---|---|---|---|
| 현재 AI 면접 세션 조회 | REQ-010 | API-025 | 자기소개서 소유권을 검증하고 현재 면접 세션의 기준 첨삭 버전, 상태, 생성 시각을 조회하거나 세션 없음 상태를 반환 |
