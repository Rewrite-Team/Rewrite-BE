...
public class KakaoLoginService {
    ...
    public KakaoAuthorizeResult authorize(FrontendTarget frontendTarget) {
        OAuthStateIssue issue = stateService.issue(frontendTarget);
    ...
    public KakaoLoginResult callback(String code, String error, String state, String browserNonce) {
        ...
        try {
            ...
            if (consumedTarget.isEmpty()) {
                ...
                return KakaoLoginResult.failed(frontendTarget);
            }
            frontendTarget = consumedTarget.orElseThrow();
            // 사용자가 로그인 취소
            if ("access_denied".equals(error) && isBlank(code)) {
                return KakaoLoginResult.canceled(frontendTarget);
            }
            ...
            return KakaoLoginResult.success(loginPersistenceService.login(kakaoUser), frontendTarget);
